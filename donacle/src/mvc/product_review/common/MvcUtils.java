package mvc.product_review.common;
...
public class MvcUtils {
	...
	public static String getPagebar(int cPage, int numPerPage, int totalContents, String url) {
		...
		// 페이지번호를 클릭했을때 링크
		url = url + "?page="; // /mvc/admin/memberList?cPage=
		...
		if(pageNo == 1) {
			...
			pagebar.append("<span class='inner_paging' onclick=goPage('" + url + (pageNo - 1) + "')>prev</span>\n");
		}
		...
		while(pageNo <= pageEnd && pageNo <= totalPage) {
			if(pageNo == cPage) {
				...
				// 현재페이지가 아닌 경우 링크를 제공.
				pagebar.append("<span class='inner_paging' onclick=goPage('" + url + pageNo + "')>" + pageNo + "</span>");
			}
		...
		if(pageNo > totalPage) {
			...
			pagebar.append("<span class='inner_paging' onclick=goPage('" + url + pageNo + "')>next</span>\n");
		}
